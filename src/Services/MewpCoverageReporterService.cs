...
namespace JsonToWord.Services
{
    public class MewpCoverageReporterService : IMewpCoverageReporterService
    {
        ...
        private static double ResolveColumnWidth(string columnName)
        {
            ...
            if (key == "customer id") return 18;
            if (key == "title (customer name)") return 42;
            if (key == "responsibility - sapwbs (esuk/il)") return 30;
            if (key == "test case id") return 16;
            if (key == "test case title") return 36;
            if (key == "number of passed steps") return 24;
            if (key == "number of failed steps") return 24;
            if (key == "number of not run tests") return 26;
            return 24;
        }
        ...
        private static bool IsNumericColumn(string columnName)
        {
            ...
            return
                key == "test case id" ||
                key == "number of passed steps" ||
                key == "number of failed steps" ||
                key == "number of not run tests";
        }
